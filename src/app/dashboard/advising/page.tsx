...
interface AdvisedStudent {
    id: string; name: string; npm: string | null;
    userCode: string; pendingItemsCount: number;
}
...
export default function AdvisingPage() {
    ...
    const { data, error, isLoading } = useSWR<PaginatedResponse>(
        token ? `${process.env.NEXT_PUBLIC_API_URL}api/advisor/students?page=${currentPage}` : null,
    ...
    if (isLoading) return <div className="container py-12 text-center">Memuat data mahasiswa...</div>;
        ...
        <div className="container py-8">
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-gray-500">Pilih mahasiswa untuk melihat dan mereview karya ilmiah mereka.</p>
                </div>
                <Link href="/dashboard/advising/add-student" className="flex items-center gap-2 py-2 px-4 border rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700">
                    <UserPlus size={16} />
                    Tambah Mahasiswa
                </Link>
            </div>
            ...
            <div className="bg-white rounded-lg border shadow-sm">
                <ul className="divide-y divide-gray-200">
                    {students.length > 0 ? students.map((student) => (
                        <li key={student.id}>
                            <Link href={`/dashboard/advising/students/${student.id}`} className="block hover:bg-gray-50">
                                        ...
                                        <div className="min-w-0 flex-1 px-4 md:grid md:grid-cols-3 md:gap-4">
                                            <div>
                                                ...
                                                <p className="mt-1 flex items-center text-sm text-gray-500">
                                                    <span className="truncate">NPM: {student.npm || '-'}</span>
                                                </p>
                                            </div>
                                            <div className="hidden md:block">
                                                <p className="text-sm text-gray-900">Kode: {student.userCode}</p>
                                            </div>
                                            <div>
                        ...
                        <div className="text-center p-16">
                            ...
                            <h3 className="mt-4 text-lg font-semibold">Belum Ada Mahasiswa Bimbingan</h3>
                            <p className="mt-1 text-gray-500">Gunakan tombol "Tambah Mahasiswa" untuk memulai.</p>
                        </div>
                    )}
                </ul>
            </div>
