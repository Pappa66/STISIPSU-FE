...
export default function EditMenuPage() {
    ...
    const { data: menuItem, error, isLoading } = useSWR<NavItem | SubMenuItem>(apiUrl, fetcher);
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        // Perbaikan URL API
        const updateUrl = `${process.env.NEXT_PUBLIC_API_URL}api/${isSubmenu ? 'submenus' : 'menu-items'}/${menuId}`;
    ...
    if (error) return <div className="text-center py-12 text-red-500">Gagal memuat data. Pastikan ID menu valid.</div>;
    if (isLoading || !menuItem) return <div className="text-center py-12">Memuat data menu...</div>;
                ...
                <div className="p-6 space-y-6 border rounded-lg bg-card text-card-foreground">
                    ...
                    <div>
                        ...
                        <fieldset className="mt-2 space-y-3">
                            ...
                            <label className="flex items-center"><input type="radio" name="type" value="STATIC_PATH" checked={type === 'STATIC_PATH'} onChange={() => setType('STATIC_PATH')} className="h-4 w-4"/><span className="ml-3 text-sm">Path Halaman Statis</span></label>
                            <label className="flex items-center"><input type="radio" name="type" value="EXTERNAL" checked={type === 'EXTERNAL'} onChange={() => setType('EXTERNAL')} className="h-4 w-4"/><span className="ml-3 text-sm">Link Eksternal</span></label>
                        </fieldset>
                    </div>
                    ...
                    {type === 'EXTERNAL' && (
                        <div><label className="block text-sm font-medium">URL Eksternal</label><input type="url" placeholder="https://contoh.com" value={href} onChange={(e) => setHref(e.target.value)} required className="w-full mt-1 px-3 py-2 border rounded-md bg-background"/></div>
                    )}
                </div>

                <div className="flex justify-end pt-4">
                    <button type="submit" disabled={isSubmitting} className="flex items-center justify-center w-36 px-4 py-2 rounded-md border bg-blue-600 text-white disabled:bg-blue-400 disabled:cursor-not-allowed">
